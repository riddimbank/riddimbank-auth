window.addEventListener('load', function() {
  ...
  var els = {
    ...
    btnMobile: document.getElementById('authToggleBtnMobile'),
  ...
  function doReg() {
    ...
    if (!c.valid) {
      ...
      var missing = [];
      if (!c.len) missing.push('• At least 8 characters');
      if (!c.up) missing.push('• Uppercase letter (A-Z)');
      if (!c.low) missing.push('• Lowercase letter (a-z)');
      if (!c.num) missing.push('• Number (0-9)');
      if (!c.spec) missing.push('• Special character (!@#$%^&*)');
      alert('Password does not meet requirements:\n\n' + missing.join('\n') + '\n\n⚠️ Note: Email cannot be changed after registration.');
      return;
    }
    ...
    firebase.auth().createUserWithEmailAndPassword(email, pw)
      .then(function(cred) {
        ...
        return cred.user.delete().then(function() {
          ...
          alert('⚠️ Please Confirm Your Email\n\n' +
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
      ...
      if (els.btnMobile) {
        els.btnMobile.textContent = 'LOG OUT';
        els.btnMobile.onclick = function() {
          if (confirm('Are you sure you want to log out?')) {
            firebase.auth().signOut().then(function() {
              clear();
              location.reload();
      ...
      // Set mobile button handler
      if (els.btnMobile) {
        els.btnMobile.textContent = 'LOG IN';
        els.btnMobile.onclick = openModal;
      }
    }
  });
